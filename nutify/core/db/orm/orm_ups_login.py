"""ORM model for login credentials and role permissions."""
...
class LoginAuth:
    ...
    def get_id(self) -> str:
        """Return user identifier for Flask-Login session storage."""
        return str(self.id)
...
def init_model(model_base, db_logger=None):
    """Initialize the LoginAuth model with the provided SQLAlchemy base."""
    global logger
    logger = db_logger
