        ...
        try:
            ...
            if LoginAuth:
                ...
                if user:
                    ...
                    user_role = getattr(user, 'role', 'administrator' if user.id == 1 else 'user')
        except Exception as e:
...
@rate_limit('12 per minute', methods=['POST'])
def api_login():
...
def api_admin_update_user_options_tabs(user_id):
    ...
    # Validate tab keys
    valid_tabs = ['email', 'extranotifs', 'webhook', 'powerflow', 'options', 'database', 'log', 'advanced', 'renamer', 'operations', 'admin']
